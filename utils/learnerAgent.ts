...
async function analyzeLosses(losses: any[]): Promise<string[]> {
    ...
    const tradesSummary = losses.map((t, i) => {
        ...
        return [
            ...
            `  Exit Type: ${t.exitType || "SELL"}`,
            `  Net Sell Value (SOL): ${t.netSellValue ?? "N/A"}`,
            `  Net ATA Close Value (SOL): ${t.netAtaCloseValue ?? "N/A"}`,
            `  Realized Exit Value (SOL): ${t.realizedExitValueSol ?? "N/A"}`,
            `  Confidence at entry: ${t.confidence}%`,
            `  Reason: ${t.reason || "N/A"}`,
            `  Exit Decision Reason: ${t.decisionReason || "N/A"}`,
            `  Status: ${t.status}`,
