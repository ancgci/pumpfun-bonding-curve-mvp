...
/**
 * Backtester Simples para Otimização de Parâmetros
 * 
 * Uso: npx ts-node tools/backtester.ts --tp=50 --sl=15
 */

// Parse CLI arguments
const args = process.argv.slice(2);
const config = {
    takeProfit: parseArgument('--tp', 40),  // Default 40%
    stopLoss: parseArgument('--sl', 25),    // Default 25%
    numTrades: parseArgument('--trades', 100), // Simular 100 trades
};
...
// Simular trades com distribuição realista
...
// Funções auxiliares
function parseArgument(flag: string, defaultValue: number): number {
...
function simulateTrades(numTrades: number, takeProfit: number, stopLoss: number) {
    let wins = 0;
    let losses = 0;
    let totalProfit = 0;
    let totalLoss = 0;
    ...
    for (let i = 0; i < numTrades; i++) {
        // Simular movimento de preço (distribuição aleatória)
        const priceChange = simulatePriceChange();

        const investmentSize = 0.05; // 0.05 SOL por trade

        if (priceChange >= takeProfit) {
            // Trade atingiu Take Profit
            ...
            // Trade neutro (não atingiu nem TP nem SL - assumir pequeno lucro/perda)
...
function simulatePriceChange(): number {
    // Simular movimento de preço com distribuição realista
    // 60% chance de trade ir bem (lucro potencial)
    // 40% chance de trade ir mal (perda potencial)

    const isWinningTrade = Math.random() < 0.6;

    if (isWinningTrade) {
        // Movimento positivo: 0% a +100%
        return Math.random() * 100;
    } else {
        // Movimento negativo: -50% a 0%
