...
| **Step 3** | Análise Técnica (`agentOrchestrator.ts`) | Snapshot técnico de 1s + score com dois regimes: completo e compacto para `pumpfun` near-migration. Classificação em `VALID`, `LOW_DATA` ou `WEAK_SETUP`. Detalhe completo em `docs/PIPELINE_3_TECHNICAL_ANALYSIS.md`. | `[Pipeline 3/8 - Technical Analysis] ✅ APROVADO | ABC (9zbDuv...) passou pela Análise Técnica Pre-LLM (Score: 65).` |
...
| **Step 5** | Hard Blocks (`agentOrchestrator.ts`) | Regras de ouro estáticas pós-decisão. Para `pumpfun` near-migration, parte dos bloqueios lentos foi aliviada e a pressão acumulada ficou menor para evitar redundância com a etapa 3 compacta. | `[Pipeline 5/8 - Hard Blocks] 🛑 REPROVADO | ABC (9zbDuv...) BLOQUEADO por regra estática (BLOCK_VWAP_DISTANCE).` |
| **Step 7** | Micro-Confirmação (`agentOrchestrator.ts`) | Observa os últimos milissegundos. Em `pumpfun` near-migration, usa janela mais curta e menos punitiva; fora disso, mantém a confirmação mais conservadora. | `[Pipeline 7/8 - Micro-Confirm] 🛑 REPROVADO | ABC (9zbDuv...) falhou na Micro-Confirmação (Sinais de Despejo).` |
